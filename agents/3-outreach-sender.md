...
**Invoke:** "Send today's outreach." Always dry-run first and show Jack before live sending.
...
1. **`PAUSED` file in repo root → stop immediately.** Report "outreach paused, nothing sent". Jack's kill switch.
...
2. Show Jack the dry-run list. **Wait for his go-ahead on the first run of any new batch.**
...
- If Gmail returns a rate/spam error, stop for the day and tell Jack. Don't retry tomorrow at a higher volume.
