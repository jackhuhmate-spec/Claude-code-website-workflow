...
## The email
Three sentences. Plain text. No greeting fluff, no bullets, no links, no images, no attachments. As if Jack typed it on his phone between jobs.

1. **The specific flaw**, named, with the borough — proof of a real look at their site. Use the `Biggest Flaw` from `leads.csv`, rewritten in Jack's voice.
...
- **No sign-off** — the sender appends "Best, Jack" and the opt-out line automatically. Adding one gives a double signature.
